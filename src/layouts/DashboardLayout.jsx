  ...
  const renderMenuItem = (item, depth = 0, parentKey = "") => {
    ...
    if (item.disabled) {
      return (
        <div
          key={key}
          className={`${sidebarCollapsed ? 'justify-center px-2' : 'gap-3 px-4'} ${paddingLeft} flex items-center py-3 text-sm text-white/40 cursor-not-allowed rounded-lg my-1`}
          title="Coming Soon"
        >
          {item.icon}
          {!sidebarCollapsed && <span>{item.label}</span>}
    ...
    const linkClasses = ({ isActive }) => {
      ...
      return `${base} py-2 text-sm ${
        isActive ? "bg-white/40 text-white font-semibold" : "text-white/80 hover:bg-white/20 hover:text-white"
      }`;
    };

    return (
      <NavLink key={key} to={item.path} className={linkClasses} onClick={() => setSidebarOpen(false)} title={sidebarCollapsed ? item.label : ''}>
        {item.icon && <span className="text-sm">{item.icon}</span>}
        {/*badge shows item.badge count when > 0 */}
        {!sidebarCollapsed && <span className="flex items-center gap-2">
